 ...
 */
class JSmallSlider extends CWidget
{
    ...
    public function init()
    {
        ...
        $this->publishAssets();
        if ($this->debug) {
            $this->registerScriptFile('js/jquery.smallslider.js', CClientScript::POS_HEAD);
        } else {
            $this->registerScriptFile('js/jquery.smallslider.js', CClientScript::POS_HEAD);
        }
        $this->registerCssFile('css/smallslider.css');
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->selector}').smallslider({$options});
SETUP;
