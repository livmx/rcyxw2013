...
class JPowerSwitch extends CWidget
{
    ...
    public function init()
    {
        ...
        //  $this->registerCssFile('css/powerSwitch.css');

        if ($this->debug) {
            $this->registerScriptFile('jquery-powerSwitch.js', CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile('jquery-powerSwitch-min.js', CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
        //jQuery('{$this->selector}').powerSwitch({$options});
        $('{$this->selector}').powerSwitch({$options});
SETUP;
