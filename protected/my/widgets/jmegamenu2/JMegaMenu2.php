...
class JMegaMenu2   extends CWidget
{
    ...
    public function init()
    {
        ...
        $this->publishAssets();
         ...
         $this->registerCssFile('stylesheets/jquery.megamenu.css');

        if ($this->debug) {
            $this->registerScriptFile('javascripts/jquery.megamenu.js', CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile('javascripts/jquery.megamenu.js', CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->selector}').megamenu({$options});
SETUP;
