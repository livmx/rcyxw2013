...
class JBubblePopup extends CWidget
{
    ...
    public function init()
    {
        ...
        $this->registerCssFile('css/jquery-bubble-popup-v3.css');

        if ($this->debug) {
            $this->registerScriptFile('scripts/jquery-bubble-popup-v3.min.js', CClientScript::POS_HEAD);
        } else {
            $this->registerScriptFile('scripts/jquery-bubble-popup-v3.min.js', CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
       $('{$this->selector}').CreateBubblePopup({$options});
SETUP;
        //> register jsCode
