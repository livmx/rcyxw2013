<?php
/**
 * wrapper the jquery megaMenu v2 for yii community
 * User: yiqing
 * Date: 13-4-17
 * Time: 上午11:42
 * To change this template use File | Settings | File Templates.
 * -------------------------------------------------------
 * @see http://www.geektantra.com/2010/05/jquery-megamenu-2/
 * -------------------------------------------------------
 */

class JDropDownMenu   extends CWidget
{
    ...
    /**
     * @var string
     * available style : style1 | style2 ;
     */
    public   $style = 'style1';

    /**
     * @return JDropDownMenu
     */
    ...
    public function init()
    {
        ...
        if( ! in_array($this->style,array('style1','style2'))){
            // correct it
            $this->style = 'style1';
            //throw new InvalidArgumentException('you must give the style1 or style2 as style param value but others ');
        ...
        $this->publishAssets();
         ...
         $this->registerCssFile("fixedMenu_{$this->style}.css");

        if ($this->debug) {
            $this->registerScriptFile('jquery.fixedMenu.js', CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile('jquery.fixedMenu.js', CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->selector}').fixedMenu({$options});
SETUP;
