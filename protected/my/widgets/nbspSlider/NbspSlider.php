 ...
 */
class NbspSlider extends CWidget
{
    ...
    public function init()
    {
        ...
        $this->publishAssets();
        if ($this->debug) {
            $this->registerScriptFile("js/jquery.nbspSlider.{$this->jsVersion}.js", CClientScript::POS_HEAD);
        } else {
            $this->registerScriptFile("js/jquery.nbspSlider.{$this->jsVersion}.min.js", CClientScript::POS_HEAD);
        }
        $this->registerCssFile('css/yii-space.css');
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->selector}').nbspSlider({$options});
SETUP;
