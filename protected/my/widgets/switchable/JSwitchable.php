...
class JSwitchable extends CWidget
{
    ...
    public function init()
    {
        ...
        if ($this->debug) {
            $this->registerScriptFile('js/jquery.switchable[all].min.js', CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile('js/jquery.switchable[all].min.js', CClientScript::POS_HEAD);
        }
        $this->registerCssFile('css/css.css');

        if (empty($this->triggerContainer)) return;
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->triggerContainer}').switchable("{$this->panelSelector}",{$options});
SETUP;
