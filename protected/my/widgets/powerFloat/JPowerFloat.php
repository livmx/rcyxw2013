 ...
 */
/**
 * @see http://www.zhangxinxu.com/study/201012/jquery-power-float-demo.html##
 *
 * Class JPowerFloat
 */class JPowerFloat extends CWidget
{
    ...
    public function init()
{
    ...
    $this->registerCssFile('css/powerFloat.css');

    if ($this->debug) {
        $this->registerScriptFile('js/jquery-powerFloat.js', CClientScript::POS_HEAD);
    } else {
        // no the min file now !
        $this->registerScriptFile('js/mini/jquery-powerFloat-min.js', CClientScript::POS_HEAD);
    }
    ...
    $jsCode .= <<<SETUP
        jQuery('{$this->selector}').powerFloat({$options});
SETUP;
