...
class JDcVerticalMegaMenu   extends CWidget
{
    ...
    public function init()
    {
        ...
        $this->registerCssFile('css/vertical_menu.css');


        // need the additional jquery plugin !
        $this->registerScriptFile('js/jquery.hoverIntent.minified.js');
        if ($this->debug) {
            $this->registerScriptFile('js/jquery.dcverticalmegamenu.1.1.js', CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile('js/jquery.dcverticalmegamenu.1.1.js', CClientScript::POS_HEAD);
        }
