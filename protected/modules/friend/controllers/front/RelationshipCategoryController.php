...
class RelationshipCategoryController extends BaseFriendController
{
	/**
	...
	{
		$model=new RelationshipCategory;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
        $model->user_id = user()->getId();

		if(isset($_POST['RelationshipCategory']))
		{
			$model->attributes=$_POST['RelationshipCategory'];
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "RelationshipCategory successfully added",
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "RelationshipCategory successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new RelationshipCategory)
     */
	...
	{
		$dataProvider=new CActiveDataProvider('RelationshipCategory');
	...
	{
		$model=new RelationshipCategory('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RelationshipCategory']))
			$model->attributes=$_GET['RelationshipCategory'];
	...
	{
		$model=RelationshipCategory::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='relationship-category-form')
		{
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = RelationshipCategory::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['RelationshipCategory'];
			if($model->validate(array_keys($_POST['RelationshipCategory']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['RelationshipCategory'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "RelationshipCategory successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = RelationshipCategory::model()->findAll($criteria);
