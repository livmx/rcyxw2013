...
class JShowCase extends CWidget
{
    ...
    public function init()
    {
        ...
        if ($this->debug) {
            ...
            $this->registerScriptFile('jquery.aw-showcase.js', CClientScript::POS_HEAD);

        } else {
            // no the min file now !
            $this->registerScriptFile('jquery.aw-showcase.min.js', CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->selector}').awShowcase({$options});
SETUP;
