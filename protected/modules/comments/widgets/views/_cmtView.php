...
<li id="comment-<?php echo $comment->cmt_id; ?>">
    ...
    if ($this->allowSubcommenting === true) {
        ...
        if ($this->allowSubcommenting === true && ($this->registeredOnly === false || Yii::app()->user->isGuest === false)) {
            echo CHtml::link(Yii::t('CommentsModule.msg', 'Add reply'), '#',
                array('rel' => $comment->cmt_id, 'class' => 'add-comment comment-reply-link',
                   // 'onclick' => "return Comment.moveForm(\"comment-{$data->cmt_id}\", \"{$data->cmt_id}\")",
                ));
        }
    }
    ?>
