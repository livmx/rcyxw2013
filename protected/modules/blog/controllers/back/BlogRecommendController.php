...
class BlogRecommendController extends BackendController
{
	/**
	...
	{
		$model=new BlogRecommend;

        $request = Yii::app()->request ;
        $objectId = $request->getParam('objectId',null);
		...
		if(isset($_POST['BlogRecommend']))
		{
			$model->attributes=$_POST['BlogRecommend'];
			if(BlogRecommend::model()->exists('object_id=:objectId',array(':objectId'=>$objectId)) || $model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "BlogRecommend successfully added"
                        )
                    );
                    return ;
                } else
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "BlogRecommend successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new BlogRecommend)
     */
	...
	{
		$dataProvider=new CActiveDataProvider('BlogRecommend');
	...
	{
		$model=new BlogRecommend('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['BlogRecommend']))
			$model->attributes=$_GET['BlogRecommend'];
	...
	{
		$model=BlogRecommend::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='blog-recommend-form')
		{
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = BlogRecommend::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['BlogRecommend'];
			if($model->validate(array_keys($_POST['BlogRecommend']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['BlogRecommend'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "BlogRecommend successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = BlogRecommend::model()->findAll($criteria);
