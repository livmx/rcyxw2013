...
class NoticePostController extends BackendController
{
	/**
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['NoticePost'];
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "NoticePost successfully added"
                        )
                    );
                    return ;
                } else
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "NoticePost successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new NoticePost)
     */
	...
	{
		$dataProvider=new CActiveDataProvider('NoticePost');
	...
	{
		$model=new NoticePost('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['NoticePost']))
			$model->attributes=$_GET['NoticePost'];
	...
	{
		$model=NoticePost::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='notice-post-form')
		{
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = NoticePost::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['NoticePost'];
			if($model->validate(array_keys($_POST['NoticePost']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['NoticePost'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "NoticePost successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = NoticePost::model()->findAll($criteria);
