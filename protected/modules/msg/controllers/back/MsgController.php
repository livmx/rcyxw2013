...
class MsgController extends BackendController
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Msg'])) {
            ...
            if ($model->save()) {
                ...
                // 根据消息类型做不同处理
                if ($model->type == Msg::TYPE_SYS_BROADCAST) {
                ...
                FROM user
                   ";
                    ...
                    $cmd = Yii::app()->db->createCommand($insertSql);
                    $cmd->execute();

                }
                 // 类型太多 用责任链设计模式！
                if($model->type == Msg::TYPE_SYS_PERSONAL){
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new Msg)
     */
    ...
    public function actionBatchDelete()
    {
        ...
        if ($request->getIsPostRequest()) {
            ...
            $models = Msg::model()->findAll($criteria);
    ...
    public function getItemsToUpdate()
    {
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id', $ids);
        $items = Msg::model()->findAll($criteria);
