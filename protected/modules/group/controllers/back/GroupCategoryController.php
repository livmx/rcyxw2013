...
class GroupCategoryController extends BackendController
{
	/**
		...
		if(isset($_POST['GroupCategory']))
		{
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "GroupCategory successfully added"
                        )
                    );
                    return ;
                } else
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "GroupCategory successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new GroupCategory)
     */
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = GroupCategory::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['GroupCategory'];
			if($model->validate(array_keys($_POST['GroupCategory']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['GroupCategory'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "GroupCategory successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = GroupCategory::model()->findAll($criteria);
