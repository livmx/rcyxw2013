...
class AdminUserController extends BackendController
{
            ...
            if ($model->save()) {
                if (Yii::app()->request->isAjaxRequest) {
                    exit(CJSON::encode(array(
                            'status' => 'success',
                            'message' => "AdminUser successfully added"
                        )
                    ));
        ...
        if (isset($_POST['AdminUser'])) {
            ...
            if ($model->save()) {
                if (Yii::app()->request->isAjaxRequest) {
                    exit(CJSON::encode(array(
                            'status' => 'success',
                            'message' => "AdminUser successfully saved"
                        )
                    ));
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new AdminUser)
     */
    ...
    public function actionBatchUpdateAjax()
    {
        ...
        if (isset($_POST['AdminUser'])) {
            $model->attributes = $_POST['AdminUser'];
            if ($model->validate(array_keys($_POST['AdminUser']))) {
                $items = $this->getItemsToUpdate();
                foreach ($items as $i => $item) {
                    $item->attributes = $_POST['AdminUser'];
                    $item->save(false); // $item->save(); will run the validate function !
                }
                exit(CJSON::encode(array(
                        'status' => 'success',
                        'message' => "AdminUser successfully saved" // .print_r($_POST['Comment'],true),
                    )
                ));
            }
        }
