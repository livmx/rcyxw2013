...
class SysFriendLinkController extends BackendController
{
	/**
	...
	{
		$model=new SysFriendLink;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['SysFriendLink']))
		{
			$model->attributes=$_POST['SysFriendLink'];
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "SysFriendLink successfully added"
                        )
                    );
                    return ;
                } else
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "SysFriendLink successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new SysFriendLink)
     */
	...
	{
		$dataProvider=new CActiveDataProvider('SysFriendLink');
	...
	{
		$model=new SysFriendLink('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SysFriendLink']))
			$model->attributes=$_GET['SysFriendLink'];
	...
	{
		$model=SysFriendLink::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='sys-friend-link-form')
		{
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = SysFriendLink::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['SysFriendLink'];
			if($model->validate(array_keys($_POST['SysFriendLink']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['SysFriendLink'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "SysFriendLink successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = SysFriendLink::model()->findAll($criteria);
