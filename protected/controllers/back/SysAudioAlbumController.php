...
class SysAudioAlbumController extends BackendController
{
	/**
	...
	{
		$model=new SysAudioAlbum;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['SysAudioAlbum']))
		{
			$model->attributes=$_POST['SysAudioAlbum'];
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(
                        array(
                            'message' => "SysAudioAlbum successfully added"
                        )
                    );
                    return ;
                } else
			...
			if($model->save()){
                if (Yii::app()->request->isAjaxRequest) {
                    $this->ajaxSuccess(array(
                            'message' => "SysAudioAlbum successfully saved"
                        )
                    );
                    return ;
                } else
     ...
     *  ajaxCreate functionality is almost using the same code (just change the $this->loadModel($id) to new SysAudioAlbum)
     */
	...
	{
		$dataProvider=new CActiveDataProvider('SysAudioAlbum');
	...
	{
		$model=new SysAudioAlbum('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SysAudioAlbum']))
			$model->attributes=$_GET['SysAudioAlbum'];
	...
	{
		$model=SysAudioAlbum::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='sys-audio-album-form')
		{
    ...
    public function actionBatchDelete()
    {
        ...
        if($request->getIsPostRequest()){
            ...
            $models = SysAudioAlbum::model()->findAll($criteria);
    ...
    public function actionBatchUpdateAjax()
	{
		...
		{
            $model->attributes=$_POST['SysAudioAlbum'];
			if($model->validate(array_keys($_POST['SysAudioAlbum']))){
                    $items=$this->getItemsToUpdate();
                    $this->onControllerAction(new ControllerActionEvent($this,$this->action->id,$items));

                    foreach($items as $i=>$item)
                    {
                        $item->attributes = $_POST['SysAudioAlbum'];
                        $item->save(false);// $item->save(); will run the validate function !
                    }
                    $this->ajaxSuccess(
                        array(
                            'message' => "SysAudioAlbum successfully saved" // .print_r($_POST['Comment'],true),
                        )
                    );
                return ;
            }
    ...
    */
    public  function getItemsToUpdate(){
        ...
        $criteria->index = 'id';
        $criteria->addInCondition('id',$ids);
        $items = SysAudioAlbum::model()->findAll($criteria);
